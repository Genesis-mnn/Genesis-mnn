...
class GenesisSimulator:
    ...
    参数:
        ...
        sleep_wake_cycle: 可选的 SleepWakeCycle 实例（v0.3 睡眠-觉醒周期模块）。
        memory_replay: 可选的 MemoryReplay 实例（v0.3 梦境重放模块）。
        synaptic_homeostasis: 可选的 SynapticHomeostasis 实例（v0.3 突触稳态模块）。
        dendritic_spine_remodeling: 可选的 DendriticSpineSleepRemodeling 实例（v0.3 树突棘睡眠重塑模块）。
    """
    ...
    def __init__(
        ...
        sleep_wake_cycle: Optional[SleepWakeCycle] = None,
        memory_replay: Optional[MemoryReplay] = None,
        synaptic_homeostasis: Optional[SynapticHomeostasis] = None,
        dendritic_spine_remodeling: Optional[DendriticSpineSleepRemodeling] = None,
    ) -> None:
        if not neurons:
        ...
        self.sleep_wake_cycle = sleep_wake_cycle
        self.memory_replay = memory_replay
        self.synaptic_homeostasis = synaptic_homeostasis
        self.dendritic_spine_remodeling = dendritic_spine_remodeling
    ...
    def reset(self, batch_size: int, device: Optional[torch.device] = None) -> None:
        ...
        # 初始化全局状态，供 sleep_wake_cycle 首次使用
        if self.global_scheduler is not None:
            _ = self.global_scheduler.step()
    ...
    def step(
        ...
        # ---- 判断当前状态（基于上一步末尾更新的状态机） ----
        is_sleep = self.sleep_wake_cycle is not None and self.sleep_wake_cycle.state == 'SLEEP'
        ...
        if is_sleep:
            ...
            # 设置神经元内部标志，避免常规前向传播中的树突棘重塑与睡眠重塑冲突
            for nrn in self.neurons:
                nrn._force_skip_intrinsic_remodeling = True

            # 步骤2：梦境重放接管神经元更新
            if self.memory_replay is not None:
                self.memory_replay.replay_step()

            # 步骤3：突触稳态衰减
            if self.synaptic_homeostasis is not None:
                self.synaptic_homeostasis.renormalize()

            # 步骤4：胶质网络步进（必须在突触衰减之后，因为胶质需要感知最新突触状态）
            if self.glia is not None:
                self.glia.step()

            # 步骤5：刷新树突棘保护缓存并执行重塑
            if self.dendritic_spine_remodeling is not None:
                self.dendritic_spine_remodeling.update_protection_from_glia()
                self.dendritic_spine_remodeling.remodel_step()

            # 根据梦境重放后的最新神经元状态更新 prev_spikes / prev_rs 缓存
            for nrn in self.neurons:
                nid = id(nrn)
                if hasattr(nrn, '_last_spike') and nrn._last_spike is not None:
                    spike = nrn._last_spike.squeeze(1).detach().to(device)
                else:
                    spike = torch.zeros(batch_size, device=device)
                self.prev_spikes[nid] = spike
                self.prev_rs[nid] = nrn.get_internal_state()['r'].clone().detach().to(device)

            # 步骤6：全局状态调度器步进（更新疲劳度、健康度等指标，为唤醒做准备）
            global_state = {}
            if self.global_scheduler is not None:
                global_state = self.global_scheduler.step()

            # 步骤7：更新睡眠-觉醒状态机（基于最新全局状态，可能唤醒）
            if self.sleep_wake_cycle is not None:
                self.sleep_wake_cycle.step(global_state)

            # 步骤8：跳过内源性学习调度器及所有常规更新（神经元前向、突触传播等）
            learn_stats = None
            ...
            for nrn in self.neurons:
                ...
                nid = id(nrn)
                spk = self.prev_spikes.get(nid, torch.zeros(batch_size, device=device))
                all_spikes.append(spk)
                u = state['u']
                all_u.append(u.squeeze(1) if u.dim() == 2 else u)
                h = state['h']
                all_h.append(h.squeeze(1) if h.dim() == 2 else h)
            ...
            # ===================== 觉醒分支 =====================
            # 清除睡眠标志，确保常规前向传播中的树突棘重塑正常执行
            for nrn in self.neurons:
                nrn._force_skip_intrinsic_remodeling = False
    ...
    def apply_modulator(self, name: str, concentration: float) -> None:
        ...
        if self.glia is not None:
            # 胶质网络内部通过 Astrocyte.modulate 处理，此处无需操作
            pass
